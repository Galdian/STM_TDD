...
def swiss_pairing(playerlist: PlayerList, rounds: int):
    roundnum = 1
    for round in range(rounds):
        ...
        while len(players_to_be_paired) > 0:
            points_required = playerlist.list_of_players[playerlist.find_player_by_starting_number(players_to_be_paired[0])].points
            tiers[tier_group] = []
            updated_players_to_be_paired = []
            tested_player = 0
            for player in players_to_be_paired:
                current_player: Player = playerlist.list_of_players[playerlist.find_player_by_starting_number(players_to_be_paired[tested_player])]
...
def sort_pairings(playerlist: PlayerList, possible_pairings: list):
    ...
    for pairing in possible_pairings:
        ...
        index += 1
    pairings.sort(key=lambda pairing: (pairing.highest_points_diff, pairing.total_points_diff, pairing.sos_diff, pairing.sosos_diff))
...
def round_robin_pairing(playerlist: PlayerList):
    ...
    for r in range(rounds):
        ...
        for p in range(pairs):
            current_pairs.append(add_pair(players_to_be_paired[first_paired_player], players_to_be_paired[second_paired_player]))
...
def resolve_round(pairs: list, playerlist: PlayerList):
    for p in pairs:
        if p.player1 == 0:
